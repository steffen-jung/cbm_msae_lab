...
Two ways to form the groups, selected by ``grouping``:
...
- ``"attention"``: S2AE's own approach -- patches are clustered per-image by
  ...
  (`attention_grouping.py`). Needs `ctx.group_labels` to be populated (i.e. a
  group-label cache built via `scripts/extract_attention_groups.py`).
"""
    ...
    def compute(self, ctx: LossContext) -> Tensor:
        if self.grouping == "tile":
            ...
            if ctx.group_labels is None:
                raise RuntimeError(
                    "group_sparsity.grouping='attention' but no group_labels were supplied to this training "
