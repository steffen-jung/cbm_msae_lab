...
def save_checkpoint(
    ...
    checkpoint: dict[str, Any] = {
        "model_state_dict": {"sae": pipeline.sae.state_dict()},
        "optimizer_state_dict": {"sae": trainer.optimizer.state_dict()},
...
def load_for_reproduction(path: str, device: str = "cpu") -> tuple[DictConfig, ConceptPipeline]:
    """Rebuilds the exact pipeline (encoder + upsampler + SAE) a
    ...
    pipeline = ConceptPipeline(encoder, upsampler, sae).to(device)
    pipeline.eval()
    return cfg, pipeline
